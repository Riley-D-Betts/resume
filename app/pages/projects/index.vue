...
<template>
  <div data-section="projects">
    <NsBreadcrumb :items="[{ label: 'Home', to: '/' }, { label: 'Projects' }]" />

    <NsPageTitle title="Projects" meta="List" subtitle="Side builds — hardware for the kids, software for the plant and beyond.">
      <template #actions>
        <a href="https://github.com/Riley-D-Betts" target="_blank" rel="noopener" class="ns-btn">↗ GitHub</a>
        <button
          type="button"
          class="ns-btn ns-btn--primary"
          @click="toast.show('New projects usually start at 11pm. This one is on the backlog.', { icon: '＋' })"
        >
          ＋ New Project
        </button>
      </template>
    ...
    <div class="ns-listwrap">
      <div class="ns-listbar">
        ...
        <input v-model="q" class="ns-input" type="search" placeholder="Search projects…" style="width: 220px" />
      ...
      <div class="ns-listfoot">
        <span>{{ rows.length }} of {{ allRows.length }} records</span>
